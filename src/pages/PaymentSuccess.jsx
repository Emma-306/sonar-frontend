...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      const beforeHash = currentUrl.split("#")[0] || "";
      const hashPart = currentUrl.split("#")[1] || "";

      const beforeHashSearch = beforeHash.includes("?")
        ? beforeHash.split("?")[1]
        : "";
      const hashSearch = hashPart.includes("?") ? hashPart.split("?")[1] : "";

      const beforeHashParams = new URLSearchParams(beforeHashSearch);
      const hashParams = new URLSearchParams(hashSearch);

      // Paystack can return the transaction reference in either part of the URL,
      // depending on whether the app is using HashRouter or a normal router.
      const reference =
        searchParams.get("reference") ||
        searchParams.get("trxref") ||
        beforeHashParams.get("reference") ||
        beforeHashParams.get("trxref") ||
      ...
      if (result.success) {
        setStatus("success");
        setMessage("Payment successful! Your Premium plan is now active.");
    ...
    verifyPayment();
  }, [searchParams, verifyPremiumPayment, navigate]);
